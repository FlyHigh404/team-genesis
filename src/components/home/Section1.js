...
export default function Section1() {
  return (
    <section
      className="w-full min-h-dvh pt-32 px-8 sm:px-12 md:px-14 lg:px-16"
      style={{
        background: `
      ...
      <div className="flex flex-col-reverse lg:flex-row items-center gap-2">
        ...
        <div className="basis-1/2 w-12/12 sm:w-11/12 md:w-10/12 flex flex-col items-start justify-center">
          <h1 className="_text-title-1 leading-tight">
            PLATFORM TERLENGKAP UNTUK KEBUTUHAN{" "}
            <span className="text-primary-n1">PERAWATAN</span> EV KAMU.
          </h1>
          <p className="w-10/12 _text-body-1 text-foreground-2 leading-tight tracking-wide block">
            Cari bengkel terpercaya, beli sparepart ORI, atur jadwal servis,
            serta rekomendasi perawatan sesuai budget untuk kondisi EV dan
            keuangan kamu, serta fitur lain{" "}
            <span className="font-bold">tanpa ribet!</span>
          </p>
          <Button
            type="button"
...
function HeroLabel({ children, ...props }) {
  return (
    <span
      {...props}
      className={`px-4 py-2 rounded-2xl backdrop-blur-xs bg-primary-n2/50 ${props.className}`}
    >
      {children}
    </span>
  );
